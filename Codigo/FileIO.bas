...
Sub LoadBalance()
    ...
    ' Validar que ListaClases este inicializado
    If ListaClases(1) = vbNullString Then
        Call TraceError(0, "ListaClases not initialized", "ES.LoadBalance", 0)
        Exit Sub
    End If
    ...
    For i = 0 To MAX_ELEMENT_TAGS - 1
        ...
        For j = 0 To MAX_ELEMENT_TAGS - 1
            ' Verificar que el indice existe en el array
            If j <= UBound(vals) Then
                ElementalMatrixForNpcs(i + 1, j + 1) = val(vals(j))
            Else
                ElementalMatrixForNpcs(i + 1, j + 1) = 1 ' Valor por defecto
            End If
        Next j
    Next i
    Set BalanceIni = Nothing
    AgregarAConsola "Se cargo el balance (Balance.dat)"
    Exit Sub
LoadBalance_Err:
...
Sub SaveUser(ByVal UserIndex As Integer, Optional ByVal Logout As Boolean = False)
    ...
    ' Save character BEFORE marking as disconnected
