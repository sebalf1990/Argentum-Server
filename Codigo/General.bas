...
Sub Main()
    ...
    With frmMain
        ...
        If IsFeatureEnabled("ShipTravelEnabled") Then
            ...
            If BarcoNavegandoForgatNix.Map > 0 Then MapInfo(BarcoNavegandoForgatNix.Map).ForceUpdate = True
            If BarcoNavegandoNixArghal.Map > 0 Then MapInfo(BarcoNavegandoNixArghal.Map).ForceUpdate = True
            If BarcoNavegandoArghalForgat.Map > 0 Then MapInfo(BarcoNavegandoArghalForgat.Map).ForceUpdate = True
        End If
    End With
...
Sub MostrarNumUsers()
    ...
    ' No hacer nada si el servidor no esta completamente iniciado
    If Not ServerInitialized Then Exit Sub
    
    On Error GoTo MostrarNumUsers_Err
    ' Solo enviar si hay conexiones activas
    If NumUsers > 0 Then
        Call SendData(SendTarget.ToAll, 0, PrepareMessageOnlineUser(NumUsers))
    End If
    On Error Resume Next ' CantUsuarios may not exist
...
Public Sub LoadDBMigrations()
    ...
    Else
        ...
        If RS.RecordCount > 0 Then LastScript = RS.Item("Date")
    End If
