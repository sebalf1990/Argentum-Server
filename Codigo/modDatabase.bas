...
' DISABLED - OLD ADODB CODE
' Private Sub Database_Connect_Async_OLD()
...
' DISABLED - OLD ADODB CODE
' Private Sub Database_Connect_OLD()
...
' DISABLED - OLD ADODB CODE
' Private Sub Database_Close_OLD()
...
' DISABLED - OLD ADODB CODE
' Private Function Query_OLD(ByVal Text As String, ParamArray Arguments() As Variant) As clsSQLite3Recordset
...
'                 Command.Parameters.Append CreateParameter_OLD(Inner, adParamInput)
'             Next Inner
'         Else
'             Command.Parameters.Append CreateParameter_OLD(Argument, adParamInput)
'         End If
'     Next Argument
...
' DISABLED - OLD ADODB CODE
' Private Function Execute_OLD(ByVal Text As String, ParamArray Arguments() As Variant) As Boolean
'     Dim Command  As New ADODB.Command
'     Dim Argument As Variant
...
' DISABLED - OLD ADODB CODE
' Private Function Invoke_OLD(ByVal Procedure As String, ParamArray Arguments() As Variant) As clsSQLite3Recordset
'     Dim Command  As New ADODB.Command
'     Dim Argument As Variant
...
' DISABLED - OLD ADODB CODE
' Private Function CreateParameter_OLD(ByVal value As Variant, ByVal Direction As Long) As Object
...
Public Function GetDBValue(Tabla As String, ColumnaGet As String, ColumnaTest As String, ValueTest As Variant) As Variant
    ...
    GetDBValue = RS.Fields(ColumnaGet)
    Exit Function
ErrorHandler:
...
Public Function GetUserValueById(CharId As Long, Columna As String) As Variant
    ...
    If RS Is Nothing Then Exit Function
    If RS.BOF Or RS.EOF Then Exit Function
    GetUserValueById = RS.Fields(Columna)
    Exit Function
GetUserValue_Err:
    Call TraceError(Err.Number, Err.Description, "modDatabase.GetUserValue", Erl)
End Function
...
Public Function GetPersonajesCountByIDDatabase(ByVal AccountID As Long) As Byte
    ...
    If RS Is Nothing Then Exit Function
    GetPersonajesCountByIDDatabase = RS.Fields(0)
    Exit Function
ErrorHandler:
...
Public Function GetPersonajesCuentaDatabase(ByVal AccountID As Long, Personaje() As t_PersonajeCuenta) As Byte
    ...
    Set RS = Query("SELECT name, head_id, class_id, body_id, pos_map, pos_x, pos_y, level, status, helmet_id, shield_id, weapon_id, guild_index, backpack_id, is_dead, is_sailing FROM user WHERE account_id = ?;", AccountID)
    If RS Is Nothing Then Exit Function
    
    i = 0
    Do While Not RS.EOF
        i = i + 1
        If i > 10 Then Exit Do ' Max 5 characters per account
        Personaje(i).nombre = RS.Item("name")
        Personaje(i).Cabeza = val(RS.Item("head_id"))
        Personaje(i).clase = val(RS.Item("class_id"))
        Personaje(i).cuerpo = val(RS.Item("body_id"))
        Personaje(i).Mapa = val(RS.Item("pos_map"))
        Personaje(i).PosX = val(RS.Item("pos_X"))
        Personaje(i).PosY = val(RS.Item("pos_Y"))
        Personaje(i).nivel = val(RS.Item("level"))
        Personaje(i).Status = val(RS.Item("Status"))
        Personaje(i).Casco = val(RS.Item("helmet_id"))
        Personaje(i).Escudo = val(RS.Item("shield_id"))
        Personaje(i).Arma = val(RS.Item("weapon_id"))
        Personaje(i).ClanIndex = val(RS.Item("Guild_Index"))
        Personaje(i).BackPack = val(RS.Item("backpack_id"))
        ...
        End If
        If val(RS.Item("is_dead")) = 1 Or val(RS.Item("is_sailing")) = 1 Then
            Personaje(i).Cabeza = 0
        End If
        RS.MoveNext
    Loop
    GetPersonajesCuentaDatabase = CByte(i)
    Exit Function
...
Public Function GetUserAmountOfPunishmentsDatabase(ByVal username As String) As Integer
    ...
    If RS Is Nothing Then Exit Function
    GetUserAmountOfPunishmentsDatabase = RS.Item("punishments")
    Exit Function
ErrorHandler:
...
Public Sub SendUserPunishmentsDatabase(ByVal UserIndex As Integer, ByVal username As String)
    ...
    If Not RS.RecordCount = 0 Then
        While Not RS.EOF
            Call WriteConsoleMsg(UserIndex, RS.Item("Number") & " - " & RS.Item("Reason"), e_FontTypeNames.FONTTYPE_INFO)
            RS.MoveNext
        Wend
    End If
    Exit Sub
...
Public Function GetUserGuildMemberDatabase(username As String) As String
    ...
    If Not RS.RecordCount = 0 Then
        ...
        i = 0
        While Not RS.EOF
            History = History & SanitizeNullValue(RS.Item("guild_name"), "")
            i = i + 1
...
Public Sub SendCharacterInfoDatabase(ByVal UserIndex As Integer, ByVal username As String)
    ...
    GuildActual = SanitizeNullValue(RS.Item("Guild_Index"), 0)
    ...
    IsLegion = RS.Item("Status") = e_Facciones.concilio Or RS.Item("Status") = e_Facciones.Caos
    IsArmy = RS.Item("Status") = e_Facciones.consejo Or RS.Item("Status") = e_Facciones.Armada
    Call WriteCharacterInfo(UserIndex, username, RS.Item("race_id"), RS.Item("class_id"), RS.Item("genre_id"), RS.Item("level"), RS.Item("gold"), RS.Item("bank_gold"), GuildRequestHistory, gName, Miembro, IsArmy, IsLegion, _
            RS.Item("ciudadanos_matados"), RS.Item("criminales_matados"))
    Exit Sub
ErrorHandler:
...
Public Function EnterAccountDatabase(ByVal UserIndex As Integer, ByVal CuentaEmail As String) As Boolean
    ...
    If Connection.State = adStateClosed Then
        ...
        Exit Function
    End If
    UserList(UserIndex).AccountID = RS.Item("Id")
...
Public Function GetCharacterIdWithName(ByVal username As String) As Long
    ...
    If Not RS Is Nothing Then
        ...
        GetCharacterIdWithName = RS.Item("Id")
        Exit Function
    End If
