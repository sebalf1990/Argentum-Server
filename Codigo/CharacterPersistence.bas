...
Public Function LoadCharacterBank(ByVal UserIndex As Integer) As Boolean
    ...
    With UserList(UserIndex)
        ...
        If Not RS Is Nothing Then
            While Not RS.EOF
                With .BancoInvent.Object(RS.Item("Number"))
                    .ObjIndex = IIf(RS.Item("item_id") <= UBound(ObjData), RS.Item("item_id"), 0)
                    If .ObjIndex <> 0 Then
                        If LenB(ObjData(.ObjIndex).name) Then
                            counter = counter + 1
                            .amount = RS.Item("amount")
                            .ElementalTags = RS.Item("elemental_tags")
                        Else
                            .ObjIndex = 0
                        End If
                    End If
                    ...
                    With .invent.Object(db_inv_slot)
                        ...
                        If .ObjIndex <> 0 Then
                            If LenB(ObjData(.ObjIndex).name) Then
                                ...
                                .Equipped = False
                                .ElementalTags = RS.Item("elemental_tags")
                                If RS.Item("is_equipped") Then
                                    Call EquiparInvItem(UserIndex, RS.Item("Number"), True)
                                End If
                            Else
                                .ObjIndex = 0
                            End If
...
#End If
    ...
    With UserList(UserIndex)
        ...
        If CLng(RS.Item("account_id")) <> .AccountID Then
        ...
        If RS.Item("is_banned") Then
            Dim BanNick As String, BaneoMotivo As String
            BanNick = RS.Item("banned_by")
            BaneoMotivo = RS.Item("ban_reason")
        ...
        If RS.Item("is_locked_in_mao") Then
        ...
        If IsFeatureEnabled("patreon_system") And (.Stats.tipoUsuario = tAventurero Or .Stats.tipoUsuario = tHeroe Or .Stats.tipoUsuario = tLeyenda) Then
...
Private Sub SetupUserBasicInfo(ByRef User As t_User, ByRef RS As clsSQLite3Recordset)
    With User
        .Id = RS.Item("Id")
        .name = RS.Item("name")
        .Stats.ELV = RS.Item("level")
        .Stats.Exp = RS.Item("Exp")
        .genero = RS.Item("genre_id")
        .raza = RS.Item("race_id")
        .clase = RS.Item("class_id")
        .Hogar = RS.Item("home_id")
        .Desc = RS.Item("Description")
        .Stats.GLD = RS.Item("gold")
        .Stats.Banco = RS.Item("bank_gold")
        .Stats.SkillPts = RS.Item("free_skillpoints")
        .pos.Map = RS.Item("pos_map")
        .pos.x = RS.Item("pos_X")
        .pos.y = RS.Item("pos_Y")
        .MENSAJEINFORMACION = RS.Item("message_info")
        .OrigChar.body = RS.Item("body_id")
        .OrigChar.head = RS.Item("head_id")
        .OrigChar.originalhead = .OrigChar.head
        .OrigChar.WeaponAnim = RS.Item("weapon_id")
        .OrigChar.CascoAnim = RS.Item("helmet_id")
        .OrigChar.ShieldAnim = RS.Item("shield_id")
        .OrigChar.BackpackAnim = RS.Item("backpack_id")
        .OrigChar.Heading = RS.Item("Heading")
        .Stats.MaxHp = RS.Item("max_hp")
        .Stats.MinHp = RS.Item("min_hp")
        .Stats.MinMAN = RS.Item("min_man")
        .Stats.MinSta = RS.Item("min_sta")
        .Stats.MinHam = RS.Item("min_ham")
        .Stats.MaxHam = 100
        .Stats.MinAGU = RS.Item("min_sed")
        .Stats.MaxAGU = 100
        .Stats.NPCsMuertos = RS.Item("killed_npcs")
        .Stats.UsuariosMatados = RS.Item("killed_users")
        .Stats.PuntosPesca = RS.Item("puntos_pesca")
        .Stats.ELO = RS.Item("ELO")
        .Stats.JineteLevel = RS.Item("jinete_level")
        .Counters.Pena = RS.Item("counter_pena")
        .ChatGlobal = RS.Item("chat_global")
        .ChatCombate = RS.Item("chat_combate")
        .Stats.Advertencias = RS.Item("warnings")
        .GuildIndex = SanitizeNullValue(RS.Item("Guild_Index"), 0)
        .LastGuildRejection = SanitizeNullValue(RS.Item("guild_rejected_because"), vbNullString)
    End With
End Sub

Private Sub SetupUserFlags(ByRef User As t_User, ByRef RS As clsSQLite3Recordset)
    With User.flags
        .Desnudo = RS.Item("is_naked")
        .Envenenado = RS.Item("is_poisoned")
        .Incinerado = RS.Item("is_incinerated")
        .Escondido = False
        .Ban = RS.Item("is_banned")
        .Muerto = RS.Item("is_dead")
        .Navegando = RS.Item("is_sailing")
        .Paralizado = RS.Item("is_paralyzed")
        .VecesQueMoriste = RS.Item("deaths")
        .Montado = RS.Item("is_mounted")
        .SpouseId = RS.Item("spouse")
        .Casado = IIf(.SpouseId > 0, 1, 0)
        .Silenciado = RS.Item("is_silenced")
        .MinutosRestantes = RS.Item("silence_minutes_left")
        .SegundosPasados = RS.Item("silence_elapsed_seconds")
        .MascotasGuardadas = RS.Item("pets_saved")
        .ReturnPos.Map = RS.Item("return_map")
        .ReturnPos.x = RS.Item("return_x")
        .ReturnPos.y = RS.Item("return_y")
    End With
End Sub

Private Sub SetupUserFactionInfo(ByRef User As t_User, ByRef RS As clsSQLite3Recordset)
    With User.Faccion
        .ciudadanosMatados = RS.Item("ciudadanos_matados")
        .CriminalesMatados = RS.Item("criminales_matados")
        .RecibioArmaduraReal = RS.Item("recibio_armadura_real")
        .RecibioArmaduraCaos = RS.Item("recibio_armadura_caos")
        .RecompensasReal = RS.Item("recompensas_real")
        .RecompensasCaos = RS.Item("recompensas_caos")
        .Reenlistadas = RS.Item("Reenlistadas")
        .NivelIngreso = SanitizeNullValue(RS.Item("nivel_ingreso"), 0)
        .MatadosIngreso = SanitizeNullValue(RS.Item("matados_ingreso"), 0)
        .Status = RS.Item("Status")
        .FactionScore = RS.Item("faction_score")
    End With
End Sub
...
Private Sub SetupUserSpells(ByRef User As t_User)
    ...
    If Not RS Is Nothing Then
        While Not RS.EOF
            User.Stats.UserHechizos(RS.Item("Number")) = RS.Item("spell_id")
            RS.MoveNext
        Wend
    End If
End Sub
...
Private Sub SetupUserPets(ByRef User As t_User)
    ...
    If Not RS Is Nothing Then
        While Not RS.EOF
            User.MascotasType(RS.Item("Number")) = RS.Item("pet_id")
            If val(RS.Item("pet_id")) <> 0 Then
...
Private Sub SetupUserBankInventory(ByRef User As t_User)
    ...
    If Not RS Is Nothing Then
        While Not RS.EOF
            With User.BancoInvent.Object(RS.Item("Number"))
                .ObjIndex = RS.Item("item_id")
                If .ObjIndex <> 0 Then
...
Private Sub SetupUserSkills(ByRef User As t_User)
    ...
    If Not RS Is Nothing Then
        While Not RS.EOF
            User.Stats.UserSkills(RS.Item("Number")) = RS.Item("value")
            RS.MoveNext
        Wend
    End If
End Sub
...
Private Sub SetupUserQuests(ByRef User As t_User)
    ...
    If Not RS Is Nothing Then
        While Not RS.EOF
            If Not IsNull(RS.Item("Number")) Then
                User.QuestStats.Quests(RS.Item("Number")).QuestIndex = RS.Item("quest_id")
                If User.QuestStats.Quests(RS.Item("Number")).QuestIndex > 0 Then
                    If QuestList(User.QuestStats.Quests(RS.Item("Number")).QuestIndex).RequiredNPCs Then
                        Dim NPCs() As String
                        NPCs = Split(RS.Item("NPCs"), "-")
                        ReDim User.QuestStats.Quests(RS.Item("Number")).NPCsKilled(1 To QuestList(User.QuestStats.Quests(RS.Item("Number")).QuestIndex).RequiredNPCs)
                        For LoopC = 1 To QuestList(User.QuestStats.Quests(RS.Item("Number")).QuestIndex).RequiredNPCs
                            User.QuestStats.Quests(RS.Item("Number")).NPCsKilled(LoopC) = val(NPCs(LoopC - 1))
                        Next LoopC
                    End If
                    If QuestList(User.QuestStats.Quests(RS.Item("Number")).QuestIndex).RequiredTargetNPCs Then
                        Dim NPCsTarget() As String
                        NPCsTarget = Split(RS.Item("NPCsTarget"), "-")
                        ReDim User.QuestStats.Quests(RS.Item("Number")).NPCsTarget(1 To QuestList(User.QuestStats.Quests(RS.Item("Number")).QuestIndex).RequiredTargetNPCs)
                        For LoopC = 1 To QuestList(User.QuestStats.Quests(RS.Item("Number")).QuestIndex).RequiredTargetNPCs
                            User.QuestStats.Quests(RS.Item("Number")).NPCsTarget(LoopC) = val(NPCsTarget(LoopC - 1))
                        Next LoopC
                    End If
                End If
            End If
...
Private Sub SetupUserQuestsDone(ByRef User As t_User)
    ...
    If Not RS Is Nothing Then
        ...
        If User.QuestStats.NumQuestsDone > 0 Then
            ...
            LoopC = 1
            While Not RS.EOF
                User.QuestStats.QuestsDone(LoopC) = RS.Item("quest_id")
                LoopC = LoopC + 1
                RS.MoveNext
            Wend
        End If
...
Public Function GetPatronTierFromAccountID(ByVal account_id) As e_TipoUsuario
    ...
    If Not RS Is Nothing Then
        ...
        tipo_usuario_db = RS.Item("is_active_patron")
...
Public Sub LoadPatronCreditsFromDB(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If Not RS Is Nothing Then
            .Stats.Creditos = RS.Item("offline_patron_credits")
        Else
            .Stats.Creditos = 0
        End If
    End With
...
Public Sub SaveCharacterDB(ByVal UserIndex As Integer)
    On Error GoTo ErrorHandler
    Call LogDatabaseError("SaveCharacterDB: CALLED for user " & UserIndex)
    ...
    With UserList(UserIndex)
        ...
        For LoopC = 1 To MAX_INVENTORY_SLOTS
            ...
            ParamC = ParamC + 6
        Next LoopC
    Call LogDatabaseError("SaveCharacterDB: About to save INVENTORY")
...
#End If
    ...
    With UserList(UserIndex)
        ...
        Dim RS As clsSQLite3Recordset
        Set RS = Query("SELECT MAX(id) as max_id FROM user")
        If RS Is Nothing Then
            .Id = 1
        Else
            .Id = val(RS.Item("max_id"))
        End If
        ' ******************* SPELLS **********************
        ReDim Params(MAXUSERHECHIZOS * 3 - 1)
        ParamC = 0
